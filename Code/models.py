...
class Model_List():
    def __init__(self, model, frames, dimensions, saved_model=None, print_model=False):
        ...
        if self.saved_model is not None:
            ...
            print("Loading Model.")
            self.model = self.CNN_DROPOUT()
        elif model == 'CNN_AVG_small':
            print("Loading Model.")
            self.model = self.CNN_AVG_small()
        elif model == 'CNN_AVG':
            print("Loading Model.")
            self.model = self.CNN_AVG()
        elif model == 'CNN_MAX':
            print("Loading Model.")
            self.model = self.CNN_MAX()
    ...
    def CNN(self):
        ...
        model = Sequential()
        model.add(Conv2D(32, (3, 3), activation='relu', padding='same', input_shape=(38, 256, 10)))
    ...
    def CNN_AVG_small(self):
        #frames_input = Input(shape=self.input_shape)
        model = Sequential()
        model.add(Conv2D(32, (1, 2), activation='relu', padding='same', input_shape=(38, 256, 10)))
        model.add(AveragePooling2D((2, 2)))
    ...
    def CNN_DROPOUT1(self):
        #frames_input = Input(shape=self.input_shape)
        model = Sequential()
        model.add(Conv2D(32, (2, 2), activation='relu', padding='same', input_shape=(38, 256, 10)))
        model.add(MaxPooling2D((2, 2)))
